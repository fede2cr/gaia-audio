...
fn is_one_shot_test_mode() -> bool {
    std::env::var("GAIA_EXIT_AFTER_ONE_BATCH")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn species_range_disabled() -> bool {
    std::env::var("GAIA_DISABLE_SPECIES_RANGE")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn analysis_trace_enabled() -> bool {
    std::env::var("GAIA_DEBUG_ANALYSIS")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn trace_analysis_step(message: impl AsRef<str>) {
    if analysis_trace_enabled() {
        info!("[analysis-trace] {}", message.as_ref());
    }
}

fn should_merge_shared_csv_names(model: &LoadedModel) -> bool {
    // Large CSV name tables (notably BirdNET+ V3) are already available
    // on the model itself and do not need to be re-expanded into the
    // shared fallback map on every file.  Skipping these huge maps keeps
    // the hot path responsive without affecting detection quality.
    model.csv_common_names().len() <= 8_000
}
...
pub fn process_file(
    ...
    // In one-shot e2e mode we skip Redis lookups entirely so the worker
    // cannot stall on coordination I/O; the test should exercise the
    // full audio path with whatever models are already loaded.
    let enabled_state = if is_one_shot_test_mode() {
        None
    } else {
        crate::kv::get_enabled_models_state()
    };
    let enabled: Vec<String> = match enabled_state {
        Some(v) => v,
        None => config.model_slugs.clone(),
    };
    ...
    let disable_species_range = species_range_disabled();
    if disable_species_range {
        info!("Species-range filtering disabled for this run (GAIA_DISABLE_SPECIES_RANGE=1)");
    }
    trace_analysis_step("building shared analysis context");

    trace_analysis_step("taxonomy init start");
    let _ = taxonomy::global();
    trace_analysis_step("taxonomy init done");
    ...
    for model in models.iter_mut() {
        if !all_enabled && !crate::manifest::slug_is_selected(&model.manifest.slug(), &enabled) {
            continue;
        }
        let preload_name = model.manifest.manifest.model.name.clone();
        let preload_slug = model.manifest.slug();
        let lang_dir = model.manifest.language_dir();
        trace_analysis_step(format!("preload-start model={preload_name} slug={preload_slug}"));
        trace_analysis_step(format!("load-language start model={preload_name} dir={}", lang_dir.display()));

        // Collect common names from every model.
        let model_names = model::load_language(
            &lang_dir, &config.database_lang,
        ).unwrap_or_default();
        trace_analysis_step(format!("load-language done model={preload_name} entries={}", model_names.len()));
        ...
        // Also include CSV-parsed common names, but avoid expanding very
        // large tables into the shared fallback map on every file.
        let csv_name_count = model.csv_common_names().len();
        trace_analysis_step(format!(
            "merge-csv-common-names model={preload_name} entries={csv_name_count}"
        ));
        if should_merge_shared_csv_names(model) {
            for (sci, com) in model.csv_common_names() {
                let norm = normalize_sci_name(sci);
                shared_common_names.entry(norm).or_insert_with(|| com.clone());
            }
            trace_analysis_step(format!(
                "merge-csv-common-names done model={preload_name} inserted_from_csv={csv_name_count}"
            ));
        } else {
            info!(
                "[analysis-trace] skipping large shared CSV name preload for {} ({} entries)",
                preload_name,
                csv_name_count,
            );
        }
        if !disable_species_range && model.has_species_range_model() {
            trace_analysis_step(format!("species-range start model={preload_name}"));
            let t0 = Instant::now();
            ...
            trace_analysis_step(format!(
                "species-range done model={preload_name} count={} elapsed={:.2}s",
                list.len(),
        ...
        trace_analysis_step(format!("preload-finished model={preload_name}"));
    }

    trace_analysis_step("shared analysis context ready");
    ...
    for model in models.iter_mut() {
        ...
        info!(
            ...
            model.manifest.manifest.model.name
        );
    ...
    // more weight than beta models (trust_weight=0.5).
    {
        let model_weights: Vec<ModelWeight> = models
            .iter()
            .filter(|m| all_enabled || crate::manifest::slug_is_selected(&m.manifest.slug(), &enabled))
            .map(|m| ModelWeight {
...
fn run_analysis(
    ...
    let db_overrides = if is_one_shot_test_mode() {
        Vec::new()
    } else {
        crate::kv::load_exclusion_overrides()
    };
    ...
    // ── read audio ───────────────────────────────────────────────────
    trace_analysis_step(format!("[{tag}] read-audio start path={}", file.file_path.display()));
    ...
    trace_analysis_step(format!("[{tag}] read-audio done chunks={}", chunks.len()));
    ...
    let own_species_list = if species_range_disabled() {
        Vec::new()
    } else {
        model.get_species_list(
            config.latitude,
            config.longitude,
            file.week(),
        )
